...
A stdlib-only (Python 3.11+) validator and renderer for the knit work-item
...
- every item ID matches `KN-<AREA>-<NNN>` and its area segment equals its `area`.
...
`check` runs in the pipeline ([`KN-TEST-002`](../registry.toml)) so an invalid or
