...
│  knit CLI ──┬── discovery   (mDNS browse on all interfaces, + short cache) │
...
One binary plays both roles. `knit up` starts the agent; every other command is
...
to keep it readable at the size knit will reach is a handful of small,
...
```
cmd/knit/            main(): flag parsing, command dispatch, usage, exit codes
internal/
  ...
  paths/               ~/.knit resolution, pidfile, logfile, cache file
```
...
peers at `~/.knit/peers.json` with a short TTL (default ~5 s). The first `run`
pays the browse; the next several are instant. `knit ls` always does a fresh
...
This is what makes `knit run` pipe-safe and script-safe. The end-to-end latency
...
6. server→client: "KNIT1 <nonce>\n"                               [proto]
7. client→server: {hmac, op:"run", cmd:[...]}\n                     [proto]
8. server: verify HMAC; spawn process; reply {"ok":true}\n          [agent]
9. client: print one dim "knit → <name>" line to stderr           [client]
