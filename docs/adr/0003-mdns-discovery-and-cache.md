...
knit feel laggy — the opposite of seamless.

## Decision
Discover peers via multicast-DNS (`_knit._tcp`), browsing on all interfaces, and
cache the resulting `name → addr:port` mapping at `~/.knit/peers.json` with a
short TTL (~5 s). `knit ls` always browses fresh and refreshes the cache. Live
capacity (load, free memory) is **never** cached — it is always obtained by a fresh
`info` probe. `--peer host:port` / `KNIT_PEERS` bypasses mDNS for filtered
...
- `KNIT_NO_CACHE` exists for anyone who wants the old always-fresh behavior.
