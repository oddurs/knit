...
share compute with zero configuration. `connex up` makes a machine discoverable
capacity; `connex run -- <cmd>` executes on whichever machine has the most spare
...
3. **Seamless** — exit codes, stdin EOF, and pipes behave byte-for-byte as local.
   `connex run` prints one dim line only when work left the machine.
...
- The plan is code: work items live in `roadmaps/registry.toml` with stable IDs
  (`CX-<AREA>-<NNN>`). Validate with `python3 roadmaps/tools/roadmap.py check` and
...
```sh
CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X main.version=$(git describe --tags --always)" -o connex ./cmd/connex
```

## Layout

- `cmd/connex` — entrypoint, dispatch, exit-code contract.
